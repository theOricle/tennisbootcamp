...
**Why**: Admin-created runnable programs can't live in static content; expiring invite holds prevent one slow payer stalling a whole group; the layers share one foundation so nothing is thrown away.
**Alternatives considered**: Static cohorts + manual coordination — the status-quo pain. Interest-driven self-filling slots first — less control over group composition; deferred.
