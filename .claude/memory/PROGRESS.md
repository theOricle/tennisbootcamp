...
### 2026-09-09 (Claude Code — intake asks age and level per participant, backlog #14)

Branch `claude/zealous-feynman-v9svd2` (fresh from `b1df66d`), draft PR. Goal: stop asking who the form is for twice, and stop asking age and level for a whole submission when a household can hold a 9-year-old and a 15-year-old. No migration (0007 already live). `/api/intake` columns 1–22 unchanged in shape and order.

- **Two steps deleted** from `src/app/intake/page.tsx`: "Who is training?" and "Where's your game right now?". The wizard is now **3 steps** — Who is this for? → When can you train? → Where can we reach you?. Everything the deleted steps left behind is gone: `whoOptionId`, the `who`/`level` branches in `isSelected` / `canContinue` / `toggleOption`, the auto-advance timer and its cleanup effect, the `OptionCard` component, the `Option` type and the `"single"`/`"multi"` step types. No dead branches remain — the footer's Next button is now unconditional.
- **Age band per person** (`src/lib/ageBand.ts`, server-safe so `household.ts` can read it): `adult` (18+) / `teen` (14–17) / `junior` (7–13), matching what the programs themselves state (camp 7–13, bootcamps 14+, group lessons 18+). It is the only source of `isMinor` now, defaults from the relationship (child → junior, everyone else → adult) and is editable. It replaces the "They're under 18" checkbox in both the guest block and "Add a person".
- **Self-estimate gains Elite** (`SELF_LEVELS`), so the retired `elite-14plus` track survives: an Elite answer still scores as `level: "elite"` and still writes `priority_score 3 / lead_type elite`. The intake requires an answer (`householdProfilesReady`) — "Not sure" is the escape hatch, as the old required step had no opt-out. Booking and enroll are untouched: they pass no `collectProfile`, so their level select stays optional.
- **Signed-in holders** get the same two questions inline under each selected participant (`collectProfile`), keyed by participant id in `HouseholdValue.profiles`; "Add a person" now actually keeps the level it collects instead of dropping it on the floor.
- **One recommendation per participant:** `recommendPrograms()` runs once per person from their age band + self-estimate (legacy `who` derived per person). One player renders exactly the screen it always did — same copy, same single card; two or more render `HouseholdMatchScreen`, a card each. `recommend.ts` gained an optional `ageBand` that gates on the programs' stated ages; with no band the legacy adult/youth gating is untouched (pinned by four `adult (level) is unchanged by the band` assertions).
- **Sheet:** cols 5–6 now describe the row's own player — `resolveSubmissionParticipant` returns `legacyWho` / `legacyLevel` from that person's band and estimate (guest blocks send `ageBand`; the signed-in chooser sends `participantProfiles` keyed by id), and the route falls back to the submission's values for any row it cannot resolve. A lone player is byte-identical to before. `test-intake-row.ts` gained three checks covering it.
- **Availability** stays one shared grid; its copy now says so ("One schedule for everyone you added — you can adjust it per person later").
- **GA:** `intake_start` and `intake_complete` keep their names; `intake_complete` now carries `participants`.
- **Verified:** `npm run lint`, `npx tsc --noEmit`, `npm run build` and all six `npm test` scripts (106 checks) green. Driven end to end in a real browser against the built dev server with `/api/intake` stubbed — one adult (3 steps, one card, Group Lessons), a Junior 9 + Teen 15 (asked once, two cards: Kids' Summer Camp and Bootcamps), and a signed-in holder with two people. 390px screenshots in `ops/reviews/2026-09-09-intake-per-participant-screenshots/`.

### 2026-09-09 (Claude Code — invite emails were failing silently)

Branch `claude/invite-email-delivery-8x2k4p` (fresh from `0932b4e`), draft PR. Reported symptom: the invite section on `/admin/cohorts/[id]` said the invites went out, and the player never got an email. No migration.

- **Root cause.** `resend.emails.send()` **resolves** with `{ data, error }` when the provider refuses a message — it does not reject. All eleven send sites in `src/lib/email.ts` did `await resend.emails.send({...})` and never read `error`, so every refusal (unverified domain, bad key, rate limit, bounce) was indistinguishable from success. `createInvites` then wrapped the call in `.catch()` — which can only catch a network throw, never the returned error — and ran `sent++` regardless, so the admin screen reported "Sent N". Proved both ways against a stubbed 403: on `origin/main` the call resolves silently; after the fix it throws `Resend refused the message to …: validation_error HTTP 403: …`.
- **One delivery path.** New private `deliver()` in `src/lib/email.ts` is the only place a message leaves the app: it reads `error` and raises `EmailSendError` with the provider's own words. All eleven sites now call it, so this was never invite-only — password-set links, booking confirmations, the recommendation email, cohort-confirmed, session-cancelled and both e-transfer emails were all equally silent.
- **`emailConfigured()`** exported alongside it. Without `RESEND_API_KEY` every send logs a stub and returns normally, which is right for local dev but must never be counted as delivered — that alone reproduces the reported symptom if the key is missing in Vercel.
- **`createInvites` returns `{ sent, emailed, errors }`.** `sent` is invite rows written (a held spot, valid token); `emailed` is what the provider accepted. A failed send keeps the row and adds a per-email error naming the reason — re-inviting issues a fresh token, which is the existing recovery path — rather than destroying a held spot.
- **The admin notice can no longer lie.** New pure `src/lib/emailResult.ts` (`formatResendError`, `inviteNotice`) is shared by the server and the admin screen. A partial or total failure renders in the error style, not as a passing note: "Held 1 spot, but no email went out. …". GA `cohort_invite_sent` now counts `emailed`, not rows written.
- **`src/scripts/test-invite-delivery.ts`** (17 checks, added to `npm test`) pins the notice wording and the invariant that a notice never claims `Sent N` when `emailed < created`.
- **Verified:** lint, `tsc --noEmit`, build and all seven `npm test` scripts (122 checks) green.
