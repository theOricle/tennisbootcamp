...
Branch `claude/intake-accounts-availability-3x9raa`, draft PR `feat(intake): account + coach leveling + availability standard (#13)`. `/api/intake` columns 1–17 byte-identical (row building extracted to `src/lib/intakeRow.ts` and pinned by `src/scripts/test-intake-row.ts`).
...
Branch `claude/etransfer-cohort-payment-l1x3b2` (fresh from main). Goal: cohort 1 is paid by e-transfer and marked paid by the coach while Stripe stays fully wired. `/api/intake` and `/api/checkout` untouched.
